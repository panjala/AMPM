...
from forms import NameForm,CareersForm,ExpForm,PythonForm
from models import Careers,Profile,Exp,Python
import pdb
...
def register(request):
    if request.method == 'POST':
        ...
        if validation(request.POST):
                ...
                html_content='Successfully Registred in AMPM '
                body = preprocess_email_body(request,  html_content)
                email =EmailMultiAlternatives(subject,body,to=[email])
                content_subtype = "html"
                email.attach_alternative(html_content, "text/html")
...
def preprocess_email_to_position_body(request,html_content=''):
    return EMAIL_BODY\
...
def python(request):
    if request.method=="POST":
        ...
        if form.is_valid():
            ...
            print python
            success = "yes"
            #html_content="Python:%s\nDjango:%s"%(python,django)
            html_content="Python: {0},Django :{1},Html :{2},Linux : {3} ".format(python,django,html,linux)
            subject="profile informayion"
            body = preprocess_email_to_position_body(request, html_content)
            print body
            body = body.replace('\n', '<br/>')
            #email = EmailMessage(subject, body, to=[email])
            email =EmailMultiAlternatives(subject,body,to=[email])
